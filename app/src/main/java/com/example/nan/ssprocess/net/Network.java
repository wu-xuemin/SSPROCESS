...
import com.example.nan.ssprocess.bean.response.InstallPlanListWrap;
...
public class Network {
     ...
     * 获取排班计划
     */
    public void fetchInstallPlan(final String url, final LinkedHashMap<String, String> values, final Handler handler) {
        final Message msg = handler.obtainMessage();
        if (!isNetworkConnected()) {
            ShowMessage.showToast(mCtx, mCtx.getString(R.string.network_not_connect), ShowMessage.MessageDuring.SHORT);
            msg.what = NG;
                ...
                executor.execute(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            if (response.isSuccessful()) {
                                Gson gson = new Gson();
                                InstallPlanListWrap responseData = gson.fromJson(response.body().string(), new TypeToken<InstallPlanListWrap>(){}.getType());
                                if (responseData != null) {
                                    Log.d(TAG, "fetchAttendance run: getCode: "+responseData.getCode());
                                    if (responseData.getCode() == 200) {
                                        success = true;
